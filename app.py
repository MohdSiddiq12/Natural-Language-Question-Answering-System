...
def pdf_extract(file_name):
    ...
    try:
        ...
        # If the file is not found locally, try to retrieve it from Firebase
        pdf_txt = get_pdf_text(file_name)
    return pdf_txt
...
def index():
    if request.method == 'POST':
        if request.form.get('btn') == 'index':
            ...
            return render_template('templates/qa.html', answer=answer, question=question)
...
def qa():
    ...
    return render_template('templates/qa.html', file_names=file_names)
